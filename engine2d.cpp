...
{
    //initFreeType();
    //face = loadFont("ChixaDemibold.ttf", 24);
    initWin(title);
    initShaders();
    mainLoop();
    //FT_Done_Face(face);
...
FT_Face Engine::loadFont(const char* fontPath, int fontSize) {
...
void Engine::renderText(FT_Face face, const char* text, glm::vec3 posScale) {
    glActiveTexture(GL_TEXTURE0);

    //posScale.z is Scale :/
    glPushMatrix();
    glLoadIdentity();
    glTranslatef(posScale.x, posScale.y, 0);
    glScalef(posScale.z, posScale.z, 1.0f);
    ...
    for (const char* p = text; *p; ++p) {
        ...
        glRasterPos2f(posScale.x + glyphSlot->bitmap_left, posScale.y - glyphSlot->bitmap_top);
        glDrawPixels(glyphSlot->bitmap.width, glyphSlot->bitmap.rows,
        GL_RED, GL_UNSIGNED_BYTE, glyphSlot->bitmap.buffer);

        posScale.x += (glyphSlot->advance.x >> 6);
    }   
...
void Engine::drawGrid(float step) {
    ...
    for (float i = -1; i <= 1; i += 0.1) {
        ...
        float aChannel;
        if (fabs(i) < epsilon) aChannel = 1.0f;
        else aChannel = 0.2f;
        //////////////////////////hLine
        //vertex
        Axis.push_back(i);
        Axis.push_back(1.0f);
        //color
        Axis.push_back(1.0f); Axis.push_back(1.0f); Axis.push_back(1.0f);
    ...
    glVertexAttribPointer(pos, 2, GL_FLOAT, GL_FALSE, 6 * sizeof(float), (void*)0);
    glEnableVertexAttribArray(pos);

    GLuint color = glGetAttribLocation(shaderProgram, "vecColor");
    glVertexAttribPointer(color, 4, GL_FLOAT, GL_FALSE, 6 * sizeof(float), (void*)(2 * sizeof(float)));
...
void Engine::drawVector(glm::vec2 vector, glm::vec3 _color) {
    std::vector<float> vertices = {
        ...
        vector.x / 10, vector.y / 10, _color.x, _color.y, _color.z, 1
    };
...
void Engine::drawVector(glm::vec2 vector, glm::vec3 _color, glm::vec2 origin) {
    std::vector<float> vertices = {
        origin.x/10, origin.y/10, _color.x, _color.y, _color.z, 1,
        origin.x/10 + vector.x/10, origin.y/10 + vector.y/10, _color.x, _color.y, _color.z, 1
    };
